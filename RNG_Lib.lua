...
pName = player.name
-- Saying hello
windower.add_to_chat(8,'----- Welcome back to your RNG.lua, '..pName..' -----')
...
\cs(255, 255, 64)${key_bind_rangedweapon} \cs(200, 200, 200)Ranged Weapon:\cr \cs(125,125,255)${player_current_rangedweapon|Accipiter}
\cs(255, 255, 64)${key_bind_flurry} \cs(200, 200, 200)Flurry:\cr \cs(125,125,255)${player_current_flurry}
]]

hub_options_std = [[ \cs(255, 115, 0)Options: \cr         
\cs(255, 255, 64)${key_bind_matchsc}\cs(200, 200, 200)Match SC Element:\cr ${player_match_sc}
\cs(255, 255, 64)${key_bind_flurry} \cs(200, 200, 200)Flurry:\cr ${player_current_flurry}
]]
...
function validateTextInformation()
	...
	main_text_hub.player_current_rangedweapon = rangedWeapon.current
	main_text_hub.player_current_flurry = flurryModes.current
...
function update_active_ja(name, gain)
    Buff['Velocity Shot'] = buffactive['Velocity Shot'] or false
    Buff['Double Shot'] = buffactive['Double Shot'] or false
	Buff['Hover Shot'] = buffactive['Hover Shot'] or false
	Buff['Decoy Shot'] = buffactive['Decoy Shot'] or false
	Buff['Camouflage'] = buffactive['Camouflage'] or false
	Buff['Overkill'] = buffactive['Overkill'] or false
...
function precast(spell,action)
	...
	if spell.english == 'Ranged' then
		if flurryModes.current == 'test' then
			equip(sets.me.preshot.test)
		elseif buffactive['Velocity Shot'] then
			if rangedWeapon.current == "Gastraphetes" then
				if flurryModes.current == '2' and buffactive[581] and buffactive[228] then
					equip(sets.me.preshot.VelGastro.FL55)
				elseif flurryModes.current == '1' and buffactive[581] and buffactive[228] then
					equip(sets.me.preshot.VelGastro.FL40)
				elseif flurryModes.current == '2' and buffactive[581] then
					equip(sets.me.preshot.VelGastro.FL30)
				elseif buffactive[228] then
					equip(sets.me.preshot.VelGastro.FL25)
				elseif flurryModes.current == '1' and buffactive[581] then
					equip(sets.me.preshot.VelGastro.FL15)
				else
					equip(sets.me.preshot.VelGastro.FL0)
				end
			elseif mainWeapon.current == "Perun +1" then
				if flurryModes.current == '2' and buffactive[581] and buffactive[228] then
					equip(sets.me.preshot.VelPerun.FL55)
				elseif flurryModes.current == '1' and buffactive[581] and buffactive[228] then
					equip(sets.me.preshot.VelPerun.FL40)
				elseif flurryModes.current == '2' and buffactive[581] then
					equip(sets.me.preshot.VelPerun.FL30)
				elseif buffactive[228] then
					equip(sets.me.preshot.VelPerun.FL25)
				elseif flurryModes.current == '1' and buffactive[581] then
					equip(sets.me.preshot.VelPerun.FL15)
				else
					equip(sets.me.preshot.VelPerun.FL0)
				end
			else
				if flurryModes.current == '2' and buffactive[581] and buffactive[228] then
					equip(sets.me.preshot.Vel.FL55)
				elseif flurryModes.current == '1' and buffactive[581] and buffactive[228] then
					equip(sets.me.preshot.Vel.FL40)
				elseif flurryModes.current == '2' and buffactive[581] then
					equip(sets.me.preshot.Vel.FL30)
				elseif buffactive[228] then
					equip(sets.me.preshot.Vel.FL25)
				elseif flurryModes.current == '1' and buffactive[581] then
					equip(sets.me.preshot.Vel.FL15)
				else
					equip(sets.me.preshot.Vel.FL0)
				end
			end
		else
			if rangedWeapon.current == "Gastraphetes" then
				if flurryModes.current == '2' and buffactive[581] and buffactive[228] then
					equip(sets.me.preshot.noVelGastro.FL55)
				elseif flurryModes.current == '1' and buffactive[581] and buffactive[228] then
					equip(sets.me.preshot.noVelGastro.FL40)
				elseif flurryModes.current == '2' and buffactive[581] then
					equip(sets.me.preshot.noVelGastro.FL30)
				elseif buffactive[228] then
					equip(sets.me.preshot.noVelGastro.FL25)
				elseif flurryModes.current == '1' and buffactive[581] then
					equip(sets.me.preshot.noVelGastro.FL15)
				else
					equip(sets.me.preshot.noVelGastro.FL0)
				end
			elseif mainWeapon.current == "Perun +1" then
				if flurrryModes.current == '2' and buffactive[581] and buffactive[228] then
					equip(sets.me.preshot.noVelPerun.FL55)
				elseif flurryModes.current == '1' and buffactive[581] and buffactive[228] then
					equip(sets.me.preshot.noVelPerun.FL40)
				elseif flurryModes.current == '2' and buffactive[581] then
					equip(sets.me.preshot.noVelPerun.FL30)
				elseif buffactive[228] then
					equip(sets.me.preshot.noVelPerun.FL25)
				elseif flurryModes.current == '1' and buffactive[581] then
					equip(sets.me.preshot.noVelPerun.FL15)
				else
					equip(sets.me.preshot.noVelPerun.FL0)
				end
			else
				if flurryModes.current == '2' and buffactive[581] and buffactive[228] then
					equip(sets.me.preshot.noVel.FL55)
				elseif flurryModes.current == '1' and buffactive[581] and buffactive[228] then
					equip(sets.me.preshot.noVel.FL40)
				elseif flurryModes.current == '2' and buffactive[581] then
					equip(sets.me.preshot.noVel.FL30)
				elseif buffactive[228] then
					equip(sets.me.preshot.noVel.FL25)
				elseif flurryModes.current == '1' and buffactive[581] then
					equip(sets.me.preshot.noVel.FL15)
				else
					equip(sets.me.preshot.Vel.FL0)
				end
			end
...
function midcast(spell,action)
	...
	elseif spell.english == 'Ranged' then
		if flurryModes.current == 'test' then
			equip(sets.me.midshot.test)
		elseif buffactive['Barrage'] then
			equip(sets.me.barrage)
		elseif buffactive['Overkill'] then
			equip(sets.me.Overkill)
		elseif buffactive['Double Shot'] then
			equip(sets.me.DoubleShot)
		elseif rangedWeapon.current == "Gastraphetes" then
			equip(sets.me.midshot.gastro)
		elseif rangedWeapon.current == "Gandiva" then
			equip(sets.me.midshot.gandiva)
		elseif rangedWeapon.current == "Fomalhaut" then
			equip(sets.me.midshot.fomo)
		else
			equip(sets.me.midshot.default)
		end
    -- Enhancing
    elseif spell.skill == 'Enhancing Magic' then
    ...
    if sets.me[spell.name] then
		...
		if spell.name == 'Wildfire' or spell.name == 'Hot Shot' then
			...
			if subWeapon.current ~= 'Malevolence' then--assumed melee range for orpheus
				if spell.element == world.weather_element and get_weather_intensity() == 2 and world.day_element ~= "Water" then
					equip(sets.midcast.Obi)
				else
					equip(sets.midcast.Orpheus)
				end
			elseif subWeapon.current == 'Malevolence' then--assumed from range so no orpheus
				if spell.element == world.weather_element and (get_weather_intensity() == 2 or world.day_element ~= "Water")  then 
					equip(sets.midcast.Obi)
				elseif spell.element == world.day_element and  world.weather_element ~= "Water"  then 
					equip(sets.midcast.Obi)
				end
			end
		end
		if spell.name == 'Trueflight' then
			...
			elseif subWeapon.current == 'Malevolence' then --assumed from range so no orpheus, leave default if don't need obi
				--check weather
				if spell.element == world.weather_element and (get_weather_intensity() == 2 or world.day_element ~= "Dark")  then 
					equip(sets.midcast.Obi)
				--check day
				elseif spell.element == world.day_element and  world.weather_element ~= "Water"  then 
					equip(sets.midcast.Obi)
		...
		if (rangedWeapon=='Accipiter' and player.TP > 1700) or (rangedWeapon=='Fomalhaut' and spell.name ~= 'Savage Blade' and player.TP > 2200) or player.TP > 2700 then
...
function self_command(command)
    ...
    if #commandArgs:split(' ') >= 2 then
        ...
        if commandArgs[1] == 'toggle' then
			...
			elseif commandArgs[2] == 'acid' then
				acid:toggle()
			...
			elseif commandArgs[2] == 'flurrymode' then
                flurryModes:cycle()
                idle()
                if use_UI == true then
                    validateTextInformation()
                else
                    windower.add_to_chat(4,"----- Flurry mode Now: "..tostring(flurryModes.value))
                end
            elseif commandArgs[2] == 'meleemode' then
                ...
                if use_UI == true then
                    ...
                    windower.add_to_chat(4,"----- Melee mode Now focus on: "..tostring(idleModes.value))
                end
			...
			elseif commandArgs[2] == 'subweapon' then
                ...
                if use_UI == true then
                    ...
                    windower.add_to_chat(4,"----- Sub Weapon set now: "..tostring(subWeapon.value))
                end
			elseif commandArgs[2] == 'rangedweapon' then
                ...
                if use_UI == true then
                    ...
                    windower.add_to_chat(4,"----- Ranged Weapon set now: "..tostring(subWeapon.value))
                end
...
function ammoSelector(spell)
	if rangedWeapon.current == "Gastraphetes" then
		if spell.english == 'Ranged' and acid.value then
			equip({ammo = AcidBolt})
			--windower.send_command('wait 1.2;gs c toggle acid')
			acid:toggle()
		else
			equip({ammo = NormalBolt})
		end
	elseif rangedWeapon.current == "Fomalhaut" or rangedWeapon.current == "Annihilator" then
		equip({ammo = NormalBullet})
	elseif rangedWeapon.current == "Accipiter" or rangedWeapon.current == "Gandiva" then
		if spell.type == "WeaponSkill" then
			if spell.name == "Flaming Arrow" or spell.name == "Piercing Arrow" or spell.name == "Dulling Arrow" or spell.name == "Sidewinder" or spell.name == "Blast Arrow"
			or spell.name =="Arching Arrow" or spell.name == "Empyreal Arrow" or spell.name == "Refulgent Arrow" or spell.name == "Jishnu's Radiance" or spell.name == "Apex Arrow" then
				equip({ammo = NormalArrow})
			else
				equip({ammo = MeleeArrow})
			end
		else
			equip({ammo = NormalArrow})
		end
