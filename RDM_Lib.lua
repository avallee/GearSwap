...
\cs(255, 255, 64)${key_bind_gain_cycle} \cs(200, 200, 200)Gain:\cr ${gain_color|\\cs(204, 0, 0)}${toggle_gain_cycle|STR} \cr
]]
...
nukes.enspell2 = {['Earth']="Enstone II", ['Water']="Enwater II",  ['Wind']="Enaero II", ['Fire']="Enfire II", ['Ice']="Enblizzard II", ['Lightning']="Enthunder II", ['Light']="", ['Dark']=""}
...
function update_active_ja(name, gain)
	...
	Buff['En-Weather II'] = buffactive[nukes.enspell2[world.weather_element]] or false
	...
	Buff['Enspell II'] = buffactive[nukes.enspell2['Earth']] or buffactive[nukes.enspell2['Water']] or buffactive[nukes.enspell2['Wind']] or buffactive[nukes.enspell2['Fire']] or buffactive[nukes.enspell2['Ice']] or buffactive[nukes.enspell2['Lightning']] or false
...
function precast(spell,action)
    ...
    if spell.type == 'WhiteMagic' or spell.type == 'BlackMagic' or spell.type == 'Ninjutsu' or spell.type == 'BlueMagic' or spell.type == 'Trust' or spell.type == 'SummonerPact' then
...
function midcast(spell,action)
    ...
    elseif spell.type == 'BlackMagic' then
		...
		elseif player.status ~= 'Engaged' and spell.skill == 'Elemental Magic' and spellMap ~= 'Helix' then
			--check weather
    ...
    if sets.me[spell.name] then
		...
		if spell.name == 'Seraph Blade' then
			-- day and weather theres enough affinity for obi to win with day + weather
			if spell.element == world.day_element and spell.element == world.weather_element then
				equip(sets.midcast.Obi)
			-- Double weather darksday doesnt matter
			elseif spell.element == world.weather_element and get_weather_intensity() == 2 and world.day_element ~= "Dark"  then
				equip(sets.midcast.Obi)
			else
				equip(sets.midcast.Orpheus)                
			end
		...
		if (subWeapon.Current == 'Thibron' and player.TP > 1900) or player.TP > 2900 then
...
function idle()
    ...
    if player.status=='Engaged' then
		if subWeapon.current:match('Shield') or subWeapon.current:match('Bulwark') or subWeapon.current:match('Buckler') or subWeapon.current:match('Forfend') then
			...
			if mainWeapon.current:match('Crocea') then
				EnspellCheck()
			end
...
function EnspellCheck()
    ...
    if Buff['En-Weather'] and get_weather_intensity() == 2 then
		if meleeModes.current == 'enspell' then
			equip(sets.midcast.Obi)
		else
			equip(sets.midcast.ObiFull)
		end
	elseif Buff['En-Weather II'] and get_weather_intensity() == 2 then
        equip(sets.midcast.Obi)
    ...
    elseif Buff['Enspell'] then
		if meleeModes.current == 'enspell' then
			...
			equip(sets.midcast.OrpheusFull)
		end
	elseif Buff['Enspell II'] then
        equip(sets.midcast.Orpheus)
    end 
	if buffactive[nukes.enspell['Earth']] or buffactive[nukes.enspell2['Earth']] then
			equip(sets.Quanpur)
	end
end
    ...
    if #commandArgs:split(' ') >= 2 then
        ...
        if commandArgs[1] == 'toggle' then
			...
			elseif commandArgs[2] == 'panic' then
				...
				while idleModes.current ~= 'shield' do
				idleModes:cycle()
				end
            end
        end
        ...
        elseif commandArgs[1]:lower() == 'nuke' then
			...
			elseif (nuke == 'gainup' or nuke == 'gaindown') then
                if nuke == 'gainup' then
                    gainStat:cycle()
                elseif nuke == 'gaindown' then 
                    gainStat:cycleback()
                end     
                if use_UI == true then                    
                    validateTextInformation()
            ...
            elseif nuke == 'gain' then
                send_command('@input /ma "'..nukes[nuke][gainStat.current]..'"') 
            else        
