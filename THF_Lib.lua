...
\cs(255, 255, 64)${key_bind_mainweapon} \cs(200, 200, 200)Main Weapon:\cr \cs(125,125,255)${player_current_mainweapon|Twashtar}
\cs(255, 255, 64)${key_bind_subweapon} \cs(200, 200, 200)Sub Weapon:\cr \cs(125,125,255)${player_current_subweapon|Centovente}
]]
...
function update_active_ja(name, gain)
    Buff['Sneak Attack'] = buffactive['Sneak Attack'] or false
    Buff['Trick Attack'] = buffactive['Trick Attack'] or false
	Buff['Perfect Dodge'] =buffactive['Perfect Dodge'] or false
end
    ...
    if sets.me[spell.name] then
		...
		if spell.name == "Rudra's Storm" then
			if buffactive['Sneak Attack'] then
				equip(sets.me.RudraSATA)
			elseif buffactive['Trick Attack'] then
				equip(sets.me.RudraSATA)
			else
				equip(sets.me["Rudra's Storm"])
			end
		end
		if spell.name == "Mandalic Stab" then
			if buffactive['Sneak Attack'] then
				equip(sets.me.MandalicSATA)
			elseif buffactive['Trick Attack'] then
				equip(sets.me.MandalicSATA)
			else
				equip(sets.me["Mandalic Stab"])
			end
		end	
		...
		if spell.name == 'Aeolian Edge' then
			...
			-- Double weather 
			elseif spell.element == world.weather_element and get_weather_intensity() == "Ice" then
					equip(sets.midcast.Obi)
			else
				equip(sets.midcast.Orpheus)                
			end
		end
		if (subWeapon.Current == 'Centovente' and player.TP > 1900) or player.TP > 2900 then
...
function aftercast(spell) 
    ...
    if spell.name == 'Sneak Attack' then
		equip(sets.me.th[meleeModes.value])
