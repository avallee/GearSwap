...
spell_maps = {
    ...
    ['Absorb-Acc']='Absorb',['Absorb-TP']='Absorb',
...
pName = player.name
-- Saying hello
windower.add_to_chat(8,'----- Welcome back to your DRK.lua, '..pName..' -----')
...
function validateTextInformation()
	...
	main_text_hub.gain_color = StatColors[gainStat.current]
...
nukes.enspell = {['Earth']="Enstone", ['Water']="Enwater",  ['Air']="Enaero", ['Fire']="Enfire", ['Ice']="Enblizzard", ['Lightning']="Enthunder", ['Light']="", ['Dark']=""}
...
function update_active_ja(name, gain)
	Buff['Last Resort'] = buffactive['Last Resort'] or false
	Buff['Arcane Circle'] = buffactive['Arcane Circle'] or false
...
function buff_change(name,gain,buff_details)
	...
	if (name == "sleep" and gain) and player.hp > 101 and player.status == "Engaged" then 
                equip(sets.Sleep)
        ...
        if (name == "terror" or name == "petrification" or name == "stun") and gain then
            equip(sets.Shit)
		elseif (namef == "terror" or name == "petrification" or name == "stun") then
			--equip_current()
        end
end
...
function midcast(spell)
    ...
    elseif spell.skill == 'Enfeebling Magic' then
		...
		if Buff['Stymie'] or Buff['Elemental Seal']then --max duration/potency
    ...
    elseif spell.type == 'BlackMagic' then
		...
		elseif player.status ~= 'Engaged' and (spell.skill == 'Elemental Magic' or spell.name:match('Drain') or spell.name:match('Aspir') or spell.name:match('Death')) and spellMap ~= 'Helix' then
			--check weather
    ...
    if spell.name:match('Absorb') and spell.name ~= 'Absorb-TP' and spell.name ~= 'Absorb-Attri' then
		if buffactive['Dark Seal'] and buffactive['Nether Void'] then
			equip(sets.midcast.DSNVAbsorb)
		elseif buffactive['Nether Void'] then
			equip(sets.midcast.NVAbsorb)
		elseif buffactive['Dark Seal'] then
			equip(sets.midcast.DSAbsorb)
		else
			equip(sets.midcast.Absorb)
		end
	elseif spell.name:match('Drain') or spell.name:match('Aspir') then
		if buffactive['Dark Seal'] and buffactive['Nether Void'] and (spell.name == 'Drain II' or spell.name == 'Drain III') then
			equip(sets.midcast.DSNVDrain)
		elseif buffactive['Nether Void'] then
			equip(sets.midcast.NVDrain)
		elseif buffactive['Dark Seal'] and (spell.name == 'Drain II' or spell.name == 'Drain III') then
			equip(sets.midcast.DSDrain)
		else
			equip(sets.midcast["Drain"])
		end
	elseif spell.name == 'Impact' and nukeModes.current == 'occult' then
    ...
    if sets.me[spell.name] then
		...
		if spell.name == 'Sanguine Blade' or spell.name == 'Shockwave' or spell.name == 'Infernal Scythe' or  spell.name =='Shadow of Death'then
		...
		if Buff['Arcane Circle'] then --if arcane circle assume fighting arcana and use founders body for ws
...
function idle()
    ...
    if player.status=='Engaged' then
		if mainWeapon.Current == 'Liberator' and buffactive['Aftermath: Lv.3'] then
...
function downgradenuke( spell )
	...
	if spell.name:match("Drain") then   
        if spell.name == "Drain III" then
            newspell = "Drain"
        elseif spell.name == "Drain II" then
            newspell = "Drain"
		elseif spell.name == "Drain" then
			newspell = ""
        end
        send_command('input /ma '..newspell..'')
    end
