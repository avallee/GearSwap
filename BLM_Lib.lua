...
enfeeb_maps = {
    ...
    ['Sleep']='maccINT', ['Sleep II']='maccINT', ['Sleepga']='maccINT', ['Sleepga II']='maccINT',
    ['Silence']='macc', 
    ['Inundation']='macc', 
    ['Dispel']='maccINT', 
    ['Break']='maccINT',['Breakga']='maccINT', 
    ['Bind']='macc', 
...
\cs(255, 255, 64)${key_bind_casting} \cs(200, 200, 200)Feet:\cr \cs(125,125,255)${player_current_feet|Herald's Gaiters}
]]
...
hub_options_lte = [[ \cs(255, 115, 0)== Options: \cr              \cs(255, 255, 64)${key_bind_matchsc}\cs(200, 200, 200)Match SC Element:\cr ${player_match_sc}            \cs(255, 255, 64)${key_bind_lock_weapon} \cs(200, 200, 200)Lock Weapon:\cr ${toggle_lock_weapon}           ]]

hub_job_lte = [[ \cs(255, 115, 0) == ${player_job}: \cr             \cs(255, 255, 64)${key_bind_element_cycle} \cs(200, 200, 200)Nuking:\cr ${element_color|\\cs(0, 204, 204)}${toggle_element_cycle|Ice} \cr            ]]
...
function midcast(spell)
    ...
    if sets.midcast[spell.name] then
		...
		if spell.name == 'Impact' and nukeModes.current == 'occult' then
    ...
    if sets.me[spell.name] then
		...
		if spell.name == 'Vidohunir' or spell.name == 'Cataclysm' then
...
function buff_change(buff,gain)
    if buff == "Mana Wall" then
        if gain then
			if mainWeapon.current == "Archmage's Staff" then
				equip(sets.buff.ManaStaff)
...
function self_command(command)
    ...
    if #commandArgs:split(' ') >= 2 then
        ...
        if commandArgs[1] == 'toggle' then
			...
			elseif commandArgs[2] == 'vroom' then
				vroom:cycle()
				idle()
                if use_UI == true then
                    validateTextInformation()
                else
                    windower.add_to_chat(4,"----- Feet set now: "..tostring(vroom.value))
                end
...
function selectSCElement()
    ...
    elseif last_skillchain.english == "Darkness" or last_skillchain.english == "Umbra"  then
        -- First we always prefer the selected element if that element can burst, if not we move on. only choose dark if manually chosen since it is very different
    ...
    elseif last_skillchain.english == "Gravitation" then
		 if elements.current == "Earth" or  elements.current == "Dark" then --keep dark if selected otherwise go earth
            return
		else
			selectedElement = "Earth"
			elements:set(selectedElement)
